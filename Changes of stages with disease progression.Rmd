...
```{r}
temp_bmr %>% filter(bmr_stage<=6) %>% ggplot(aes(feature_delta,bmr_stage,col=factor(first_bmr),group=factor(SubjectID)))+
  geom_line(alpha=0.2)+
...
```{r}
temp_king_mitos %>% filter(king<=5) %>% ggplot(aes(feature_delta,king,col=factor(first_king),group=factor(SubjectID)))+
  geom_line(alpha=0.2)+
...
```{r}
temp_king_mitos %>% filter(mitos<=5) %>% ggplot(aes(feature_delta,mitos,col=factor(first_mitos),group=factor(SubjectID)))+
  geom_line(alpha=0.2)+
